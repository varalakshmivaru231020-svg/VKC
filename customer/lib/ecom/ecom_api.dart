...
/// Auth state + token storage for the vkcgoldikshu `/v1` API.
...
class EcomAuth {
  ...
  ///
  /// The identity is cached beside the tokens and restored offline; /auth/me
  /// is a background refresh, and only the server rejecting the token
  /// (401/403) ends the session.
...
/// True for the transport failures that mean "no connection", so screens can
/// show the offline state rather than a generic error.
...
/// [fallback] for transport errors that carry no body. Never leaks a raw
/// exception string.
String ecomError(Object e, String fallback) {
  if (e is DioException) {
    ...
    if (data is Map) {
      ...
      if (msg is String && msg.trim().isNotEmpty && msg.length < 200) return msg;
    }
...
/// Client for the vkcgoldikshu `/api/v1/*` ecommerce API.
class EcomApi {
  EcomApi._() {
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onError: (e, handler) async {
        ...
        // /auth/me must NOT be skipped: its 401 is what should spend the
        // refresh token, otherwise a returning customer is logged out.
  ...
  // Store artwork and the category list change on a campaign, not per visit;
  // re-fetching them every time a tab mounted only delayed the first paint.
  static const _ttl = Duration(minutes: 10);
  final Map<String, (DateTime, Object)> _cache = {};

  Future<T> _cached<T>(String key, Future<T> Function() fetch, {bool force = false}) async {
    final hit = _cache[key];
    if (!force && hit != null && DateTime.now().difference(hit.$1) < _ttl) return hit.$2 as T;
    final value = await fetch();
    _cache[key] = (DateTime.now(), value as Object);
    return value;
  }
  ...
  Future<AuthSession> verifyOtp(String phone, String otp, {String? name}) async {
    ...
    // and push the local basket up so it follows the customer.
  ...
  Future<ProductPage> products({
    ...
    final r = await _dio.get('/products', queryParameters: {
      ...
      if (categorySlugs.isNotEmpty) 'categorySlugs': categorySlugs.join(','),
  ...
  /// Published reviews + the average, for the product page.
  Future<ReviewPage> reviews(String slug, {int page = 1, int limit = 5}) async {
    final r = await _dio.get('/products/$slug/reviews', queryParameters: {'page': page, 'limit': limit});
    return ReviewPage.fromJson((r.data as Map).cast<String, dynamic>());
  }

  /// Categories, cached for the session. Pull-to-refresh passes [force].
  Future<List<EcomCategory>> categories({bool flat = true, bool force = false}) =>
      _cached('categories:$flat', force: force, () async {
  ...
  /// Store bootstrap config (contact, shipping, legal). GET /app-config
  ...
  Future<List<HeroSlide>> heroSlides({bool force = false}) => _cached('hero', force: force, () async {
        final r = await _dio.get('/hero-slides');
        final list = (r.data['slides'] ?? r.data['heroSlides'] ?? []) as List;
        return list.map((s) => HeroSlide.fromJson((s as Map).cast<String, dynamic>())).toList();
      });
  ...
  /// Merchandising banners (GET /banners), cached; filter by slot in memory.
  Future<List<Banner>> banners({String? position, bool force = false}) async {
    final all = await _cached('banners', force: force, () async {
      final r = await _dio.get('/banners');
      return (r.data['banners'] as List? ?? const [])
          .map((b) => Banner.fromJson((b as Map).cast<String, dynamic>()))
          .toList();
    });
    final list = all.where((b) => position == null || b.position == position).toList();
  ...
  /// Photos and videos from Admin → Gallery (GET /gallery).
  Future<List<GalleryItem>> gallery({int limit = 120, bool force = false}) =>
      _cached('gallery:$limit', force: force, () async {
        final r = await _dio.get('/gallery', queryParameters: {'limit': limit});
        return (r.data['items'] as List? ?? const [])
            .map((g) => GalleryItem.fromJson((g as Map).cast<String, dynamic>()))
            .where((g) => g.url.isNotEmpty)
            .toList();
      });

  /// Customer testimonials (GET /testimonials).
  Future<List<Testimonial>> testimonials({bool force = false}) => _cached('testimonials', force: force, () async {
        final r = await _dio.get('/testimonials');
        return (r.data['testimonials'] as List? ?? const [])
            .map((t) => Testimonial.fromJson((t as Map).cast<String, dynamic>()))
            .where((t) => t.quote.trim().isNotEmpty)
            .toList();
      });

  /// The brand story block (GET /about).
  Future<AboutContent> about({bool force = false}) => _cached('about', force: force, () async {
        final r = await _dio.get('/about');
  ...
  Future<void> syncCart(List<CartItem> items) async {
    await _dio.post('/cart', data: {
      'items': items.map((i) => {'productId': i.productId, 'variantId': i.variantId, 'quantity': i.quantity}).toList(),
    });
  }

  // ── Payment methods ─────────────────────────────────────────────────────
  /// Which payment methods the store has switched on, straight from the
  /// website's own endpoint (GET /api/payment-config) — the SAME source the
  /// website checkout reads, so Admin → Settings → Payments is the single
  ...
  /// Promote an address to the customer's default.
