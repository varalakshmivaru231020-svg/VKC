 ...
 * Public — creates a Video Shopping appointment request. Store staff follow up
 * by phone/WhatsApp to confirm a video-call link; view submissions via
 * `npm run db:studio` (VideoBooking table) until a dedicated admin view exists.
 */
export async function POST(req: Request) {
  const { name, phone, email, preferredDate, preferredTime, notes } = await req.json();
