...
export function WhyChoose() {
  ...
  return (
    <section className="relative overflow-hidden" style={{ background: `linear-gradient(160deg, ${C.bark} 0%, ${C.barkSoft} 60%, ${C.green} 140%)` }}>
      ...
      <div aria-hidden className="absolute right-[-3%] bottom-[-10%] font-heading select-none pointer-events-none hidden lg:block" style={{ fontSize: "clamp(9rem,20vw,18rem)", lineHeight: 1, color: "rgba(240,201,109,0.05)", letterSpacing: "-0.05em", fontStyle: "italic" }}>1988</div>
      ...
      <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-40">
              ...
              <motion.div
                ...
                style={{ background: "rgba(252,251,247,0.05)", border: "1px solid rgba(240,201,109,0.25)", backdropFilter: "blur(6px)" }}
              >
                <motion.div key={active} initial={reduced ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: EASE }}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-heading" style={{ fontSize: 46, lineHeight: 1, color: "rgba(240,201,109,0.35)", letterSpacing: "-0.04em" }}>0{active + 1}</span>
                  </div>
                  <div className="font-heading mt-6" style={{ fontSize: 24, lineHeight: 1.12, color: C.ivory }}>{REASONS[active].t}</div>
                  <p className="font-body mt-2" style={{ fontSize: 14.5, lineHeight: 1.7, color: "rgba(252,251,247,0.72)", textAlign: "left", hyphens: "none" }}>{REASONS[active].d}</p>
                  {REASONS[active].tag && (
                    <span className="inline-block mt-4 font-body font-semibold rounded-full px-3 py-1.5" style={{ fontSize: 12, letterSpacing: "0.06em", background: "rgba(240,201,109,0.14)", color: C.jaggeryLite, border: "1px solid rgba(240,201,109,0.3)" }}>
                      {REASONS[active].tag}
                    </span>
                  )}
                </motion.div>
              ...
              <Link href="/about" className="group mt-7 inline-flex items-center gap-2 font-body font-semibold text-sm" style={{ color: C.jaggeryLite }}>
                Read our story
                <span className="grid place-items-center h-8 w-8 rounded-full transition-transform duration-300 group-hover:rotate-45" style={{ border: "1px solid rgba(240,201,109,0.45)" }}>
          ...
          <ul className="lg:col-span-7 list-none m-0 p-0" style={{ borderTop: "1px solid rgba(240,201,109,0.16)" }}>
            {REASONS.map((r, i) => {
              const on = i === active;
              return (
                <motion.li
                  ...
                  style={{ borderBottom: "1px solid rgba(240,201,109,0.16)" }}
                >
                  {/* sweep */}
                  <motion.span aria-hidden className="absolute inset-0 rounded-md origin-left" animate={{ scaleX: on ? 1 : 0, opacity: on ? 1 : 0 }} transition={{ duration: 0.5, ease: EASE }}
                    style={{ background: "linear-gradient(90deg, rgba(240,201,109,0.12), transparent 70%)" }} />
                  <motion.span className="relative font-heading tabular-nums" animate={{ scale: on ? 1.5 : 1, color: on ? C.jaggeryLite : "rgba(252,251,247,0.4)" }} transition={{ duration: 0.4, ease: EASE }}
                  ...
                  <div className="relative">
                    ...
                    <motion.p animate={{ x: on ? 8 : 0, opacity: on ? 1 : 0.6 }} transition={{ duration: 0.4, ease: EASE }}
                      className="font-body mt-1.5" style={{ fontSize: 14.5, lineHeight: 1.6, color: "rgba(252,251,247,0.75)", maxWidth: 560, textAlign: "left", hyphens: "none" }}>
                      {r.d}{r.tag ? ` ${r.tag}.` : ""}
                    </motion.p>
                  </div>
                  <motion.div className="relative h-12 w-12 rounded-full grid place-items-center" animate={{ background: on ? C.jaggery : "rgba(240,201,109,0.08)", color: on ? C.bark : C.jaggeryLite, rotate: on ? 12 : 0 }} transition={{ duration: 0.4, ease: EASE }}
                    style={{ border: "1px solid rgba(240,201,109,0.3)" }}>
